...
void simulation_passiveScalar(double **u, double **u_new, double **v, double **v_new, double **F, double **G, double **P, double **P_new, double **Phi, double **Phi_new, int nx, int ny, double Re, double dx, double dy, double dt){
  for(int i = 1; i <= nx-3; i++){
    for(int j = 1; j <= ny-3; j++){
      ...
      UV = (u[i][j]/(2*dx))*(Phi[i+1][j]-Phi[i-1][j]) + (v[i][j]/(2*dy))*(Phi[i][j+1]-Phi[i][j-1]);
      PHI = (1/Re)*(Phi[i+1][j] - 2*Phi[i][j] + Phi[i-1][j])*(1/(2*dx)) +  (1/Re)*(Phi[i][j+1] - 2*Phi[i][j] + Phi[i][j-1])*(1/(2*dy));
      Phi_new[i][j] = ((PHI - UV)*dt)+Phi[i][j];


	//      Phi_new[i][j] = (((((Phi[i+1][j]-2*Phi[i][j]+Phi[i-1][j])/pow(dx,2))*(1/Re)-((u[i][j]/(2*dx))*(Phi[i+1][j]-Phi[i-1][j])))+(((Phi[i][j+1]-2*Phi[i][j]+Phi[i][j-1])/pow(dy,2)*Re)-((v[i][j]/(2*dy))*(Phi[i][j+1]-Phi[i][j-1]))))*dt+Phi[i][j];
